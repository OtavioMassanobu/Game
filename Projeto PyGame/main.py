    ...
    def boss_timer(self):
        ...
        if self.boss_spawn_time <= 0:
            ...
            if self.boss and randint(0, 50) == 0:
                self.boss.sprite.shoot(self.enemy_shoots, SCREEN_HEIGHT, self.shoot_sound)
    ...
    def collision(self): #verificacao das colisões
        ...
        if self.player.sprite.shoots:
            ...
            if pygame.sprite.spritecollide(shoot, self.boss, False):
                big_bullet = self.boss.sprite.take_damage()
                shoot.kill()
            if big_bullet:
                self.enemy_shoots.add(big_bullet)


            '''if pygame.sprite.spritecollide(shoot, self.boss, False):
                shoot.kill()
                boss = self.boss.sprite
                boss.take_damage()  # O boss recebe dano
                if boss.health == 1:
                    big_bullet = boss.shoot_big_bullet()
